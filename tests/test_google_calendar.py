...
mock_events_result = {"items": [mock_event]}
...
def test_initialize_connection(
    mock_open, mock_exists, mock_flow, mock_build, mock_credentials
):
...
def test_format_event_times():
    ...
    start_time_formatted, end_time_formatted = google_calendar.format_event_times(
        start, end, local_tz
    )
